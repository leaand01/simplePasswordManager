...
If multiple users are created, the fact that two users cannot have identical login credendials, ensures that good intended users do not accidentically gain access to other accounts, mitigating the risk of a confidentiality breach. However, for an attacker this features does not mitigate or lower the risk of successfull brute force attack.
