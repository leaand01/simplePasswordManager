...
If multiple users are created, the fact that two users cannot have identical login credentials, ensures that good intended users do not accidentally gain access to other accounts, mitigating the risk of a confidentiality breach. Further, the user would only know he entered an existing user’s login credentials by reading the code as the application only tells him the entered username is taken. However, for an attacker this feature does not mitigate or lower the risk of a successful brute force attack.
